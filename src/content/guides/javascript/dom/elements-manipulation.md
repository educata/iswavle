...
თუ მნიშვნელობა არ აქვს მაშინ დააბრუნებს `null`-ს.
...
ამ მაგალითში დაკლიკების შემდგომ დაილოგება ღილაკის ატრიბუტების მნიშვნელობები, შეგვეძლო სამომხმარებლო ფუნქციის ნაცვლად
`arrow` ფუნქციით დაგვეწერა მაგრამ მსგავსი მიდგომით `this` დავკარგავდით.
...
მეთოდი ღებულობს ორ ტექსტურ პარამეტრს: ატრიბუტის სახელი და ატრიბუტის მნიშვნელობა. მეთოდი არაფერს არ აბრუნებს.
...
ატრიბუტის წაშლა. მეთოდი ღებულობს ერთ ტექსტურ პარამეტრს თუ რომელი ატრიბუტი წაშალოს. მეთოდი არაფერ არ აბრუნებს.
...
ელემენტს გააჩნია თუ არა ატრიბუტი. მეთოდი ღებულობს ერთ ტექსტურ სტრინგს, რომელსაც შეამოწმებს გააჩნია თუ არა. მეთოდი დააბრუნებს
`true` იმ შემთხვევაში თუ ატრიბუტი გააჩნია ხოლო `false` თუ არ გააჩნია.
...
[`attributes`](https://developer.mozilla.org/en-US/docs/Web/API/Element/attributes) თვისება დააბრუნებს ყოველ ატრიბუტს, რაც გააჩნია ელემენტს.
დაბრუნებული მნიშვნელობა იქნება [`NamedNodeMap`](https://developer.mozilla.org/en-US/docs/Web/API/NamedNodeMap) ტიპის.
...
რომლებიც მეტწილადად იწყება `data-`-თი. დაბრუნებული მნიშვნელობა იქნება [`DOMStringMap`](https://developer.mozilla.org/en-US/docs/Web/API/DOMStringMap) ტიპის.
...
რის მიხედვითაც გაეშვება CSS ანიმაცია. კლასის თვისებები და მეთოდები თავმოყრილია ერთ ობიექტში
[`classList`](https://developer.mozilla.org/en-US/docs/Web/API/Element/classList), რომელზეც წვდომა შეგვიძლია ელემენტის ამოღების
შემდგომ.
...
ღებულობს კლასის დასახელებებს სტრინგის სახით და აბრუნებს არაფერს, უბრალოდ ელემენტზე დაამატებს კლას ან კლასებს.
...
კონკრეტულ მაგალითში ღილაკზე დაკლიკების შემდგომ დივი, რომელსაც `addClassContainer` აქვს `id`-ათ მიიღებს კლას `animate`.
...
ღებულობს კლასის დასახელებებს სტრინგის სახით და აბრუნებს არაფერს, უბრალოდ ელემენტზე წაშლის კლას ან კლასებს.
...
ამ მაგალითში კი განიხილება ღილაკზე დაკლიკების შემდგომ `animate` კლასის წაშლა დივიდან.
...
პროცესი, როცა მნიშვნელობა არ აქვს მიენიჭოს ან პირიქით თუ აქვს მოეხსნას. კონკრეტულად ამ მეთოდისთვის თუ აქვს წაუშლის თუ არ აქვს დაუმატებს.
მეთოდი პარამეტრად ღებულობს კლასის დასახელებას სტრინგის სახით და დააბრუნებს `true` ან `false`, იმის მიხედვით თუ დაემატა `true` თუ ამოშალა `false`.
...
არის მომენტები როცა გვჭირდება ელემენტზე კლასის შემოწმება გააჩნია თუ არა. [`contains`](https://developer.mozilla.org/en-US/docs/Web/API/DOMTokenList/contains)-ის
მეთოდიც კი ამისთვის გვაქვს, რომ შევამოწმოთ ელემენტს გააჩნია თუ არა სპეციფიკური კლასი. მეთოდი პარამეტრად ღებულობს სტრინგს და დააბრუნებს
`true` თუ შეიცავს ხოლო `false` თუ არ შეიცავს.
...
სტატიაში განვიხილეთ ელემენტებზე მანიპულაციის სხვადასხვა მეთოდები, რაც გვეხმარება დინამიური კონტენტის შექმნაში.
