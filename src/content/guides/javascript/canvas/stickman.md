...
თამაშის იდეა მარტივია, გვაქვს ერთი მოთამაშე, რომელიც უსასრულოდ გადაადგილება მარცხნივ ან მარჯვნივ. თამაში არის 2D ტიპის.
...
ამ შედეგის მისაღწევად კი უნდა შევქმნათ პარალქსის ეფექტი.
...
მცირედი სტილიზაცია, რომ კანვასის ელემენტი იყოს ცენტრში და არ ჰქონდეს `overflow` (ტექნიკურად `800px` ვუწერთ კონფიგურაციაში ზომას, თუმცა შესაძლებელია სრულიად რესპონსიული სახითაც შესრულება).
...
- `x` - პირველი სურათის კორდინატი
- `x2` - მეორე სურათის კორდინატი
- `moveSpeedX` - სურათის მოძრაობის სიჩქარე

რეალურად გვაქვს ერთი სურათი მაგრამ მას ვიყენებთ ორჯერ. ერთი სურათის გამოყენებით რესურს (კადრს) საკმაოდ მალევე ამოვწურავთ, ხოლო როცა ორ სურათს ვიყენებთ შეგვიძლია იგივე სურათი გავამეოროთ პირველის დასრულების შემდგომ.

`initCanvasSizes` ფუნქცია გაეშვება ერთხელ და მიანიჭებს კანვას კონფიგში არსებულ ზომებს. `drawBackground` ფუნქცია კი დახატავს სურათს ორჯერ. `animate` ფუნქციის გამოყენებით კი ვციკლავთ ჩვენს კოდს, რომ პარალაქსის ეფექტმა უსასრულოდ იმუშავოს.

`animate` ფუნქციაში არსებული ლოგიკური გამოსახულებები ემსახურება იმ ნაწილს თუ რა დროს უნდა მოხდეს `x` კორდინატის ცვლილება პირველი სურათისთვის და `x2` კორდინატის ცვლილება მეორე სურათისთვის. ამავე ფუნქციაში ხდება ბექგრაუნდის დახატვა არსებული `x` და `x2` კორდინატებით, ასევე `requestAnimateFrame` ფუნქციის გამოძახება, რომ გამუდმებით გამოიძახოს `animate` ფუნქცია და მივიღოთ ერთგვარი ლუპი.

## მოძრაობის დამატება

არსებული ანიმაცია მიდის უსასრულოდ მარჯნივ. უფრო დინამიური რეზულტატისთვის საჭიროა მოძრაობების შემოღება მომხარებლისთვის. შემოვიტანოთ <kbd>&#8592;</kbd> და <kbd>&#8594;</kbd> ისრების მოძრაობები.
...
არსებული კოდით უკვე შეგვიძლია ორივე მიმართლებით მოძრაობა ისრების გამოყენებით. ამისათვის კი დაგვჭირდა კონფიგურაციის მოდიფიცირება დაემატა ორი ახალი თვისება: `originalMoveSpeedX` და `isMoving`.
...
ასევე დაემატა ორი ივენთის მოსმენა კლავშზე დაკლიკება და ხელის აშვება. როცა მომხარებელს კლავიშზე ხელი აქვს დაჭერილი ამ დროს რომელიმე მიმართულებით უნდა მოხდეს გადაადგილება, ხოლო გაშვების შემთხვევაში შეწყდეს ანიმაცია.

არსებულ შემთხვევაში `animate` ფუნქციას ვიძახებთ იმ შემთხვევაში თუ მომხარებელს კლავიშზე ხელი აქვს დაკლიკებული.

## მოთამაშის შემოტანა

ეხლა კი საჭიროა დავამატოთ მოთამაშეც. თავდაპირევლად შემოვიტანოთ მარტივი ფიგურა, რომელიც პასუხისმგებელი იქნება მოძრაობაზე.
...
- `player` - მოთამაშის ობიექტი.
  - `x` - მოთამაშის `x` კორდინატი.
  - `y` - მოთამაშის `y` კორდინატი.
...
შევქმენით `drawPlayer` ფუნქცია, რომელიც პასუხიმგებელია მოთამაშის დახატვაზე. ფუნქციაში გამოგვაქვს წითელი მართკუთხედი, რომელსაც დავუწეთ საზღვრები თუ რა ნაწილს არ უნდა გაცდეს კანვასისა.

ასევე განახლება გავუკეთეთ `keydown` ივენთ ლისთენერს, რისი განახლებითაც უკვე ვიმახსოვრებთ მომხარებლის სამოძრაო მიმართულებას.

## მოთამაშის ახტომა

ეხლა კი დავუმატოთ ახტომის ფუნქციონალი. ახტომის შემოტანა მოისაზრებს მცირედი გრავიტაციის ფიზიკის შემოტანას.
...
- `isJumping` - ბულეანის მნიშვნელობა, მოთამაშე ჰაერშია თუ არა.
...
დაცემის ეფექტისთვის, ყოველ კადრზე ხდება მოთამაშის `y` კორდინატის ცვლილება ახტომის აჩქარების მნიშვნელობით და ახტომის აჩქარებაც იცვლება ყოველ კადრში გრავიტაციის მნიშვნელობიდან გამომდინარე. თუ საწყის წერტილს დაუბრუნდა მოთამაშის ჰაერში ყოფნის თვისება შეიცვლება `false`-ზე.

შეიცვალა `keydown` ივენთის ლისთენერიც. უკვე ვიმასხოვრებთ თუ მოთამაშემ, რომელ ღილაკებს დააჭირა და მის მიხედვით ვიწყებთ სხვადასხვა ფუნქციონალის გაშვებას. თუ ერთდროულად აკლიკებს: <kbd>&#8592;</kbd> <kbd>&#8594;</kbd> და <kbd>&#8593;</kbd> კლავიშებს, მაშინ არ განვიხილავთ არანაირ მოქმედებას, ველოდებით მოთამაშის საწყის `y` პოზიციაზე დაბრუნებას.

## მოთამაშის დახატვა

ჩვენი თამაშის დასასრულებლად საჭიროა რეალური მოთამაშის ხატულა და არა წითელი მართკუთხედი. გამოვიყენოთ კანვასის თვისებები და დავხატოთ ლამაზი stickman.
...
ამ სტატიაში განვიხილეთ თუ როგორ უნდა აგვეწყო მცირედი თამაში stickman JavaScript-ს გამოყენებით. რეალურად მხოლოდ მოძრაობები არის განხილული თუმცა სხვადასხვა პლატფორმა და მოქმედებები სურვილისამებრ შეგიძლიათ დაუმატოთ. თითოეული კოდის დაწერა განსხვავებული მიდგომითაც შეიძლება, დამოკიდებულია უშუალოდ დეველოპერზე და არსებულ კოდზე. კარგი ჩელენჯი იქნება თქვენთვის მცირედი პლატფორმის შემოტანა, რაზეც მოთამაშეს შეეძლება გადასვლა და სხვა მცირედი დეტალების შემოტანაც.
